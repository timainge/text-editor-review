...
- [x] **Expanded the editor comparison to fully include KendoReact** (done 2026-06-15) → `research/editor-comparison.md`. Added a dedicated §4 "What It Took: KendoReact" (parallel to TipTap §2 / React Email §3), gave Kendo equal treatment in the §5 table, §6 long-term (content lock-in: Kendo's plain-HTML output is the *best* of the three on lock-in), §7 steelman, and §8 open items; refreshed all rows now that the exhibit is Playwright-verified (removed the stale "not browser-verified" caveats).
  - **Maintainer clarification incorporated:** if adopted, **KendoReact would be the *sole* editor**. So the ProseMirror-singleton crashes (§9.1) — which only exist because this repo also ships TipTap — are scored as a *comparison-repo artifact, NOT an adoption cost*; a Kendo-only app needs no `overrides` and loads cleanly. This clears Kendo's one reliability mark but leaves the commercial-license and footprint negatives intact (those don't shrink when it's the only editor).
  - **Recommendation: unchanged — headless TipTap** for a greenfield, single-field, no-existing-license email editor (footprint, zero recurring cost, neutral storage). KendoReact is the right call for a team **already on KendoReact**, or one that explicitly values vendor support/SLA over self-ownership. See `research/editor-comparison.md` §1.1.
